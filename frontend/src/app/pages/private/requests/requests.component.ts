...
export class RequestsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.api.getRequests().then((res: any) => {
      this.pendingRequests = res.filter((request: any) => request.status === 'En attente');
      this.acceptedRequests = res.filter((request: any) => request.status === 'Acceptée');
      this.refusedRequests = res.filter((request: any) => request.status === 'Refusée');
    })
  }
